...
pub fn random_scene_light(aspect: f32) -> Scene {
    let mut rng = thread_rng();

    let mut objects: Vec<Box<dyn Hittable>> = Vec::new();

    let checker_tex = Box::new(CheckerTex {
        odd: Box::new(ConstantTex { color: vec(0.2, 0.3, 0.1)}),
        even: Box::new(ConstantTex { color: vec(0.9, 0.9, 0.9)}),
        scale: 1.0
    });    
    ...
    for a in -11..11 {
        for b in -11..11 {
            ...
            if choose_mat < 0.8 {
                ...
                // metal
                let albedo = Box::new(ConstantTex {color: random_vec_range(0.5, 1.0)});
    ...
    Scene {
        ...
        environment: Box::new(Environment { emit: Box::new(ConstantTex { color: vec(0.1, 0.05, 0.05)})})
    }
}